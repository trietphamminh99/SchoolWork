...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		if (!file)
		{
			std::cout << "AppErrors::CannotOpenFile " << argv[1];
			return 1;
		}
		do
		{
	...
	auto bookMod = [&](Book& bookLambda) {

		if (bookLambda.country().compare("US") == 0)

			bookLambda.price() *= usdToCadRate;

		if (bookLambda.country().compare("UK") == 0 && (bookLambda.year() >= 1990 && bookLambda.year() <= 1999))

			bookLambda.price() *= gbpToCadRate;
	...
	for (auto i = 3; i < argc; ++i)
	{
		...
		{
			std::cout << "** EXCEPTION: " << errorLine <<endl;
		}
	}
	if (argc < 3) {
